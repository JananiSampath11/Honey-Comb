...
function Desserts(){
    ...
    const [num,setNum]=useState(desserts.map(item=>0));
    const [disable,setDisable]=useState(desserts.map(item=>false));
...
<div className="w-full flex items-center justify-center h-[80vh]">
        ...
        <div className=" w-3/4 h-full xl:w-3/4 lg:w-3/4 md:w-3/4 sm:w-3/4 xs:w-3/4 xxs:w-3/4 bg-white bg-opacity-90 " >
        <Link to ="/Cart">
            ...
            <div className=" w-full h-full overflow-y-scroll hover:scrollbar-thumb-green-700">
                {desserts.map(({name,description,price,image},i)=>(
